...
library(zipcode); data(zipcode); library(data.table); zipcode = as.data.table(zipcode); 
library(maps); library(deldir)
setkey(zipcode,zip)


dt = read.csv("label(new).csv", header = F)
...
map("state")
i=64
points(loc[dt$clus==i], col = dt$clus[dt$clus==i], pch = 19, cex = 0.5)
dev.off()
##group dt by zipcode
getmode <- function(v) {
   uniqv <- unique(v)
...
tab = tab[sort(tab$freq, index.return = T, decreasing = T)$ix,]
colnames(tab) = c("clus", "freq")
tab1 = tab[tab$freq>40,]
...
clust = dt$clus
# dl = deldir(x,y)
dl = deldir(c(loc$longitude,-1000,-1000,1000,1000),c(loc$latitude,-1000,1000,1000,-1000))
clust = c(clust, rep(length(clust)+1,4))


pdf(file = "Regions_64.pdf", height = 50, width = 100)
plot.dl(dl,clust)

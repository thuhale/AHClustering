#This function allows for mapping hrr on USA map on a given scale (ruler)
...
#  are easily visible.
color.igraph.scale = function(g, ruler){
	colors <- brewer.pal(9, "YlOrRd") #set breaks for the 9 colors 
	brks<-classIntervals(ruler*100, n=9, style="quantile")
	brks<- brks$brks #plot the map

	col=colors[findInterval(V(g)$weight*100, brks,all.inside=TRUE)]
	return(col)
}
...
plot.dl = function(dl, clust, res = 1, indeg, ruler){
 ...
 # aggregate with clust
	...
	V(gclust)$weight = indeg
 # make a coloring
 	cols = color.igraph(gclust, ruler)
